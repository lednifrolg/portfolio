...
export function useTypingAnimation(
  ...
  // Derived rather than stored: typing is complete once every line has been consumed.
  const isTypingComplete = shouldStartTyping && currentLine >= terminalText.length;

  useEffect(() => {
    if (!shouldStartTyping) return;
    if (currentLine >= terminalText.length) return;
